...
# Class defining the Berth Allocation Problem (BAProblem)
class BAProblem(search.Problem):
    ...
    # Method to solve the problem using uniform cost search
    def solve(self):
        node = uniform_cost_search(self)  # Perform uniform cost search on the problem
        
        if node is not None:  # If a solution is found
            actions = node.solution()  # Get the list of actions from the solution
            solution = [(-1, -1)] * self.n  # Initialize the solution with empty (-1, -1) for each vessel

            # Apply the actions to the solution (set mooring times and positions)
